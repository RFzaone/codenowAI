...
export default function App() {
  ...
  useEffect(() => { fetch("/api/event", { method:"POST", headers:{"content-type":"application/json"}, body:JSON.stringify({event:"site_open"}) }).catch(()=>{}); }, []);
  ...
  async function redeem() {
    ...
    try { const res=await fetch("/api/redeem",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({code:redeemCode,userId:profile.userId})}); const data=await res.json(); if(!res.ok) throw new Error(data.error||"Redemption failed"); setProfile(p=>({...p,plan:data.plan})); setRedeemCode(""); setRedeemStatus(`Activated ${PLAN_NAMES[data.plan as Plan]}!`); } catch(e) { setRedeemStatus(e instanceof Error?e.message:"Redemption failed."); }
  }
