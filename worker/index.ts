...
async function ensureSchema(db?: D1Database) {
  ...
  await db.prepare(`CREATE TABLE IF NOT EXISTS user_plans (
    user_id TEXT PRIMARY KEY,
    plan TEXT NOT NULL CHECK(plan IN ('free','plus','pro')),
    updated_at TEXT NOT NULL
  )`).run();
}
function planRank(plan: Plan) { return plan === 'pro' ? 3 : plan === 'plus' ? 2 : 1; }
async function getUserPlan(db: D1Database | undefined, userId: string): Promise<Plan> {
  if (!db || !userId) return 'free';
  await ensureSchema(db);
  const row = await db.prepare('SELECT plan FROM user_plans WHERE user_id = ?').bind(userId).first<{plan: Plan}>();
...
async function handleChat(request: Request, env: Env) {
  ...
  try {
    ...
    const storedPlan = userId !== "anonymous" ? await getUserPlan(env.DB, userId) : 'free';
    const plan = env.DB ? storedPlan : requestedPlan;
...
async function handleEntitlement(request: Request, env: Env) {
  const url = new URL(request.url);
  const userId = String(url.searchParams.get('userId') || '').trim().slice(0, 128);
  if (!userId) return withCors(json({ plan: 'free' }));
  try {
    return withCors(json({ plan: await getUserPlan(env.DB, userId) }));
  } catch (error) {
    return withCors(json({ error: error instanceof Error ? error.message : 'Could not load plan.' }, 500));
  }
}
...
async function handleRedeem(request: Request, env: Env) {
  ...
  try {
    ...
    const current = await getUserPlan(env.DB, userId);
    const finalPlan = planRank(row.plan) > planRank(current) ? row.plan : current;
    await env.DB.prepare("INSERT INTO user_plans (user_id, plan, updated_at) VALUES (?, ?, ?) ON CONFLICT(user_id) DO UPDATE SET plan = excluded.plan, updated_at = excluded.updated_at")
      .bind(userId, finalPlan, new Date().toISOString()).run();
    await bump(env, "redemptions");
    return withCors(json({ ok: true, plan: finalPlan }));
  } catch (error) {
